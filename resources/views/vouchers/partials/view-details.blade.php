        ...
        <div class="col-md-4">
            <div class="card h-100">
                <div class="card-body">
                    ...
                    <p class="card-text small">{{ Str::limit($vr->ticket->title, 50) }}</p>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card h-100">
                <div class="card-body">
                    ...
                    <h5>{{ $vr->attachments->count() }} photo(s)</h5>
                    <p class="card-text small">Uploaded with this request</p>
                </div>
            </div>
        </div>
    </div>
                    ...
                    @foreach ($vr->attachments as $photo)
                        <div class="col-md-4 col-sm-6">
                            <div class="photo-item">
                                <img src="{{ Storage::url($photo->file_path) }}" class="img-fluid rounded"
                                    alt="{{ $photo->file_name }}"
                                    style="width: 100%; height: 200px; object-fit: cover; cursor: pointer;"
                                    onclick="window.open('{{ Storage::url($photo->file_path) }}', '_blank')">
                                <div class="small text-muted mt-1 text-center">{{ $photo->file_name }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
